...
(function () {
    ...
    function move(curE, objTarget, duration, effect, callback) {
        var aryEffect = ["Linear", "Bounce-easeIn", "Bounce-easeOut", "Bounce-easeInOut"];
        var tempEffect = myEffect.Linear;
        if (typeof effect === "number") {
            var str = aryEffect[effect % aryEffect.length];
            var arrStr = str.split("-");
            tempEffect = arrStr.length >= 2 ? myEffect[arrStr[0]][arrStr[1]] : myEffect[arrStr[0]];
        }
        ...
        curE.timer = setInterval(function () {
            ...
            for (var attr in objTarget) {
                //console.log(tempEffect(time, begin[attr], change[attr], duration));
                utils.css(curE, attr, tempEffect(time, begin[attr], change[attr], duration));
            }
        }, 10)
    }
